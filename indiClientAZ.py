...
class IndiClient(PyIndi.BaseClient):
    ...
    def connectLocalServer(self, logging_level=logging.INFO, port = 7624):
        """Connect to a local INDI server with Logging Level. This is useful for testing purposes."""

        logging.basicConfig(format = '%(asctime)s %(message)s', level = logging_level)

        # Create an instance of the IndiClient class and initialize its host/port members
        self.setServer("localhost", port)

        # Connect to server
        print("Connecting and waiting 1 sec")
        if not self.connectServer():
            print(f"No indiserver running on {self.getHost()}:{self.getPort()} - Try to run")
        ...
        self.logger.info("Connected to local INDI server")
